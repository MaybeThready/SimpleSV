...
本说话人识别模型采用的数据集是CN-Celeb1。该数据集包含了大量的中文说话人语音样本，适用于中文说话人识别任务。该数据集主要分为训练数据集和测试数据集两部分，每个数据集中的语音样本均为16kHz采样率的单声道音频文件，平均每个说话人包含约135.24个语音样本。本项目将原生学习数据集（共108188条数据）按照99:1的比例划分为训练集（共107106条数据）和验证集（共1082条数据）。测试数据集主要包含声纹注册数据（共196条数据）和声纹验证数据（共17777条数据）。
...
运行`main.py`即可启动程序。初始化后，模型默认提供8种指令：
...
- Test model: 测试模型。该命令会使用最终模型在测试集上进行准确率测试，并输出测试结果，包括EER、EER对应的阈值以及DET曲线数据文件。请确保运行该命令前已经生成了最终模型文件。
- Show DET curve: 显示DET曲线。该命令会使用测试阶段生成的DET曲线数据文件绘制并显示DET曲线图，曲线颜色越绿代表阈值越高，越蓝代表阈值越低。请确保运行该命令前已经生成了DET曲线数据文件。
